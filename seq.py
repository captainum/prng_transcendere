...
def seq(u: list, x: list, d: int, bb: int, b: int, k: int):
    def func_w(n: int):
        return (
            (
            ...
            (d * n + x[0]) * (d * n + x[1]) * (d * n + x[2]) * (d * n + x[3]) * (bb ** n)
        )
    ...
    for n in range(0, h + 1):
        if len(delta_up) == 0:
            alpha_up.append(b * 0 * w_down[n] + pow(b, n) * 1 * w_up[n])
            alpha_down.append(1 * w_down[n])
        else:
            alpha_up.append(b * delta_up[n - 1] * w_down[n] + pow(b, n) * delta_down[n - 1] * w_up[n])
    ...
    while a[h] >= b - 1:
        ...
        alpha_up.append(b * delta_up[h - 1] * w_down[h] + pow(b, h) * delta_down[h - 1] * w_up[h])
...
with open("tests/output_davasilchenko_10.txt", "w") as f:
    f.write(''.join(map(str, a[1:k])))
